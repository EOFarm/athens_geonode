...
Since this application uses geonode, base source of settings is ``geonode.settings`` module. It provides defaults for many items, which are used by geonode. This application has own settings module, ``athens_geonode.settings``, which includes ``geonode.settings``. It customizes few elements:
 ...
 * Adds ``athens_geonode`` to installed applications, updates templates, staticfiles dirs, sets urlconf to ``athens_geonode.urls``.

Whether you deploy development or production environment, you should create additional settings file. Convention is to make ``athens_geonode.local_settings`` module. It is recommended to use ``athens_geonode/local_settings.py``.. That file contains small subset of settings for edition. It should:
...
::
    ...
    |   geonode.settings     |     ->      |  athens_geonode.settings    |      ->     |  athens_geonode.local_settings |
    +------------------------|-------------|-------------------------------|-------------|----------------------------------+
